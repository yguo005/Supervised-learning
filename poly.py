...
# Load the dataset
csv_path = 'boneage2/boneage_data.csv'
if not os.path.exists(csv_path):
    raise FileNotFoundError(f"CSV file not found: {csv_path}")

data = pd.read_csv(csv_path)
print(f"CSV file loaded. Number of records: {len(data)}")
...
for index, image_id in enumerate(data['id']):
    if 1377 <= image_id <= 2005:  # Ensure the image ID is within the specified range
        image_path = f'boneage2/{image_id}.png'
...
if data.empty:
    raise ValueError("The dataset is empty after filtering for valid images. Please check the data source.")
...
polynomial_regression = Pipeline([
    ...
    ('polynomial', PolynomialFeatures(degree=2)),  # Set a fixed degree to reduce complexity
...
grid_search = GridSearchCV(polynomial_regression, param_grid, cv=3, scoring='neg_mean_squared_error', n_jobs=-1)  # Reduce cv to 3
